...
SQLite database.  To avoid touching the real ``~/.bud/``:

* ``bud.commands.config_store._BUD_ROOT`` is patched (via conftest autouse
  fixture) to a pytest ``tmp_path`` so all path functions resolve to isolated
  temp directories.
* ``bud.commands.db_commands.get_engine`` is patched to return an async engine
  backed by the same tmp_path file so all SQLAlchemy operations go to the
...
def db_file(tmp_path):
    """Return a Path for a test database file under the isolated bud root."""
    user_dir = tmp_path / "users" / "default"
    user_dir.mkdir(parents=True, exist_ok=True)
    return user_dir / "bud.db"
...
class TestDbInit:
    ...
    def test_init_exits_successfully(self, runner, db_file, db_url):
        with patch("bud.commands.db_commands.get_engine", new=_make_test_get_engine(db_url)), \
...
class TestDbPushPullRegistered:
    ...
    def test_push_requires_db_to_exist(self, runner, tmp_path):
        user_dir = tmp_path / "users" / "default"
        user_dir.mkdir(parents=True, exist_ok=True)
        # bud.db intentionally does not exist
    ...
    def test_pull_requires_bucket_configured(self, runner, tmp_path):
        user_dir = tmp_path / "users" / "default"
        user_dir.mkdir(parents=True, exist_ok=True)
        config_file = user_dir / "config.json"
