...
def list_projects():
    ...
    async def _run():
        ...
        async with get_session() as db:
            items = await project_service.list_projects(db, user_id)
            if not items:
                click.echo("No projects found.")
                return
            rows = [[str(p.id), p.name, "Yes" if p.is_default else ""] for p in items]
            click.echo(tabulate(rows, headers=["ID", "Name", "Default"], tablefmt="postgres"))
...
def create_project(name):
    ...
    async def _run():
        user_id = require_user_id()
        async with get_session() as db:
            p = await project_service.create_project(db, ProjectCreate(name=name), user_id)
